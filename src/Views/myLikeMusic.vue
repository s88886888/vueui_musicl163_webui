...
export default {
  ...
  methods: {
    //   async  getlikeMuiscid() {
    //       const userid = window.sessionStorage.getItem("userid");
    //       const { data: res1 } = await this.$http.get(
    //         "/user/playlist?uid=" + userid
    //       );

    //       this.userLikeMuiscid = res1.playlist[0].id;
    //       console.log(this.userLikeMuiscid);
    //     },
    ...
    async commenthot() {
      ...
      const { data: res } = await this.$http.get("/comment/hot", {
        params: { id: res1.playlist[0].id, type: 2 },
      });
      if (res.code !== 200) {
        return this.$message.error(
    ...
    async commentplaylist() {
      ...
      const { data: res } = await this.$http.get(
        "/comment/playlist?id=" + res1.playlist[0].id
      );
      if (res.code !== 200) {
        return this.$message.error(
